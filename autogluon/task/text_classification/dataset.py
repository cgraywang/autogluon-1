...
class MRPCTask(AbstractGlueTask):
    ...
    def __init__(self):
        ...
        self.metric.add(Accuracy())
        self.metric.add(F1(average='micro'))
        self.org_dataset_train = GlueMRPC(segment='train')
        self.org_dataset_dev = GlueMRPC(segment='dev')
        self.org_dataset_test = GlueMRPC(segment='test')
        self.train_sampler, self.dev_sampler = get_split_samplers(self.org_dataset_train, split_ratio=0.8)
        super(MRPCTask, self).__init__(class_labels, self.metric, is_pair)
    ...
    def get_dataset(self, segment='train'):
        ...
        """
        if segment == 'train':
            return SampledDataset(self.org_dataset_train, self.train_sampler)
        elif segment == 'dev':
            return SampledDataset(self.org_dataset_train, self.dev_sampler)
        elif segment == 'org_dev':
            return self.org_dataset_dev
        elif segment == 'test':
            return self.org_dataset_test
        else:
            raise NotImplementedError
    ...
    def dataset_org_dev(self):
        ...
        tuple of str, Dataset : the segment name, and the dataset.
        """
        return 'org_dev', self.get_dataset(segment='org_dev')
...
class RTETask(AbstractGlueTask):
    ...
    def __init__(self):
        ...
        self.metric = Accuracy()
        self.org_dataset_train = GlueRTE(segment='train')
        self.org_dataset_dev = GlueRTE(segment='dev')
        self.org_dataset_test = GlueRTE(segment='test')
        self.train_sampler, self.dev_sampler = get_split_samplers(self.org_dataset_train, split_ratio=0.8)
        super(RTETask, self).__init__(class_labels, self.metric, is_pair)
...
class STSBTask(AbstractGlueTask):
    ...
    def __init__(self):
        is_pair = True
        class_labels = None
        self.metric = PearsonCorrelation(average='micro')
        self.org_dataset_train = GlueSTSB(segment='train')
        self.org_dataset_dev = GlueSTSB(segment='dev')
        self.org_dataset_test = GlueSTSB(segment='test')
        self.train_sampler, self.dev_sampler = get_split_samplers(self.org_dataset_train, split_ratio=0.9)
        super(STSBTask, self).__init__(class_labels, self.metric, is_pair)
...
class SSTTask(AbstractGlueTask):
    ...
    def __init__(self):
        ...
        class_labels = ['0', '1']
        self.metric = Accuracy()
        self.org_dataset_train = GlueSST2(segment='train')
        self.org_dataset_dev = GlueSST2(segment='dev')
        self.org_dataset_test = GlueSST2(segment='test')
        self.train_sampler, self.dev_sampler = get_split_samplers(self.org_dataset_train, split_ratio=0.8)
